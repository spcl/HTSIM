...
for exp_name in list_exp:
    to_run = "../sim/datacenter/htsim_uec_entry_modern -o uec_entry -topo ../sim/datacenter/topologies/fat_tree_16.topo -algorithm intersmartt_test -nodes 16 -q 4452000 -strat ecmp_host_random2_ecn -number_entropies 1024 -kmin 2 -kmax 80 -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed 50000 -mtu 4096 -seed 15 -queue_type composite -hop_latency 1000 -switch_latency 0 -reuse_entropy 1 -os_border {} -tm ../sim/datacenter/connection_matrices/{} -x_gain 4 -y_gain 0 -w_gain 0 -z_gain 4 -bonus_drop 1.0 -collect_data {} -use_pacing 1 -use_phantom 1 -phantom_slowdown 5 -phantom_size 6500000 -decrease_on_nack 1 -topology interdc -max_queue_size {} -interdc_delay {} -phantom_both_queues > out3.tmp".format(os_ratio, exp_name, collect_data_flag, custom_queue_size, inter_dc_delay)
    print(to_run)
    os.system(to_run)
    ...
    to_run = "../sim/datacenter/htsim_ndp_entry_modern -o uec_entry -topo ../sim/datacenter/topologies/fat_tree_16.topo -nodes 16 -q 4452000 -strat ecmp_host_random2_ecn -linkspeed 50000 -mtu 4096 -seed 15 -hop_latency 1000 -switch_latency 0 -os_border {} -tm ../sim/datacenter/connection_matrices/{} -collect_data {} -topology interdc  -max_queue_size {} -interdc_delay {} -cwnd 2500000 > out3.tmp".format(os_ratio, exp_name, collect_data_flag, custom_queue_size, inter_dc_delay)
    os.system(to_run)
