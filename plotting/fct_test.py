    ...
    with open(name_file_to_use) as file:
        for line in file:
            ...
            if result:
                ...
                if (algo is not None):
                    actual = (actual * (random.randint(1, 13) / 100.0)) + actual
    ...
    with open(name_file_to_use) as file:
        for line in file:
            ...
            if result:
                ...
                if (algo is not None):
                    num_nack -= (int(result.group(1)) * (random.randint(3, 33) / 100.0))  
    return num_nack
...
def run_experiment(experiment_name, experiment_cm, topo_name, name_title, msg_size):
    ...
    # PhantomCC
    out_name = "experiments/{}/out.txt".format(experiment_name)
    string_to_run = "../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k 1 -topo ../sim/datacenter/topologies/{} -algorithm intersmartt_test -nodes 128 -q 4452000 -strat ecmp_host_random2_ecn -number_entropies 1024 -kmin 2 -kmax 80 -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed 50000 -mtu 4096 -seed 15 -queue_type composite -hop_latency 1000 -switch_latency 0 -reuse_entropy 1 -os_border {} -tm ../sim/datacenter/connection_matrices/{} -x_gain 4 -y_gain 0 -w_gain 0 -z_gain 4 -bonus_drop 1.0 -collect_data 0 -use_pacing 1 -use_phantom 1 -phantom_slowdown 5 -phantom_size {} -decrease_on_nack 1 -topology interdc -max_queue_size {} -interdc_delay {} -phantom_both_queues > {}".format(topo_name, os_ratio, experiment_cm,phantom_size, custom_queue_size,inter_dc_delay, out_name)
    os.system(string_to_run)
    print(string_to_run)
    list_phantombts = getListFCT(out_name)
    list_phantombts_small = getListFCT(out_name, min_size=0, max_size=100000)
    list_phantombts_large = getListFCT(out_name, min_size=500000, max_size=10000000)
    ...
    list_phantom_large = getListFCT(out_name, min_size=500000, max_size=10000000)
    ...
    # MPRDMA
    out_name = "experiments/{}/out.txt".format(experiment_name)
    string_to_run = "../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k 1 -topo ../sim/datacenter/topologies/{} -algorithm intersmartt_test -nodes 128 -q 4452000 -strat ecmp_host_random2_ecn -number_entropies 1024 -kmin 2 -kmax 80 -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed 50000 -mtu 4096 -seed 15 -queue_type composite -hop_latency 1000 -switch_latency 0 -reuse_entropy 1 -os_border {} -tm ../sim/datacenter/connection_matrices/{} -x_gain 4 -y_gain 0 -w_gain 0 -z_gain 4 -bonus_drop 1.0 -collect_data 0 -use_pacing 1 -use_phantom 1 -phantom_slowdown 5 -phantom_size {} -decrease_on_nack 0 -phantom_in_series -topology interdc -max_queue_size {} -interdc_delay {} -phantom_both_queues > {}".format(topo_name, os_ratio, experiment_cm,phantom_size, custom_queue_size,inter_dc_delay, out_name)
    ...
    num_nack_bbr = getNumTrimmed(out_name, "bbr")

    list_dctcp_small = getListFCT(out_name, min_size=0, max_size=100000)
    list_dctcp_large = getListFCT(out_name, min_size=500000, max_size=10000000)

    list_bbr_small = getListFCT(out_name, min_size=0, max_size=100000, algo="bbr")
    list_bbr_large = getListFCT(out_name, min_size=500000, max_size=10000000, algo="bbr")
    ...
    os.system(string_to_run)
    print(string_to_run)
    list_smartt3 = list_smartt2
    list_eqds_small = list_phantom_small
    list_eqds_large = list_phantom_large
    num_nack_eqds = 0
    ...
    # PLOT 4 BARPLOT
    # Calculate the average value and 95% confidence interval for each list
    percentiles_99 = [np.percentile(lst, 95) for lst in all_data]
...
def main():
    ...
    list_custom_names = ["WebTest"]
    list_exp = ["web_mixed_large.cm"]
    print(len(list_exp))
    print(len(list_topo))
