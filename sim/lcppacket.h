...
// LcpPacket and LcpAck are subclasses of Packet.
// They incorporate a packet database, to reuse packet objects that are no
// longer needed. Note: you never construct a new LcpPacket or LcpAck directly;
// rather you use the static method newpkt() which knows to reuse old packets
// from the database.

class LcpPacket : public Packet {
  public:
    ...
    inline static LcpPacket *newpkt(PacketFlow &flow, const Route &route,
                                    seq_t seqno, seq_t dataseqno, int size,
                                    bool retransmitted = false,
                                    uint32_t destination = 99) {
        LcpPacket *p = _packetdb.allocPacket();
        p->set_route(
    ...
    inline static LcpPacket *newpkt(LcpPacket &source) {
        LcpPacket *p = _packetdb.allocPacket();
    ...
    void free() {
        // printf("Packet (LcpPacket) being freed ID is %d - From %d\n", id(),
        //        from);
        // fflush(stdout);
  ...
  protected:
    ...
    bool _syn;
    simtime_picosec _ts;
    static PacketDB<LcpPacket> _packetdb;
    bool _retransmitted;
};

class LcpAck : public Packet {
  public:
    typedef LcpPacket::seq_t seq_t;

    LcpAck() : Packet(){};

    inline static LcpAck *newpkt(PacketFlow &flow, const Route &route,
                                 seq_t seqno, seq_t ackno, seq_t dackno,
                                 uint32_t destination = UINT32_MAX) {
        LcpAck *p = _packetdb.allocPacket();
    ...
    void free() {
        // printf("Packet (LcpAck) being freed ID is %d - From %d\n", id(),
        // from); fflush(stdout);
        _packetdb.freePacket(this);
    }
    inline seq_t seqno() const { return _seqno; }
  ...
  protected:
    ...
    simtime_picosec _ts;
    static PacketDB<LcpAck> _packetdb;
};

class LcpNack : public Packet {
  public:
    typedef LcpPacket::seq_t seq_t;

    LcpNack() : Packet(){};

    inline static LcpNack *newpkt(PacketFlow &flow, const Route &route,
                                  seq_t seqno, seq_t ackno, seq_t dackno,
                                  uint32_t destination = UINT32_MAX) {
        LcpNack *p = _packetdb.allocPacket();
        p->set_route(flow, route, acksize, ackno);
        p->_bounced = false;
        p->_type = LCPNACK;
        p->_seqno = seqno;
    ...
    void free() {
        // printf("Packet (LcpNack) being freed ID is %d - From %d\n", id(),
        // from); fflush(stdout);
        _packetdb.freePacket(this);
    }
    inline seq_t seqno() const { return _seqno; }
  ...
  protected:
    ...
    simtime_picosec _ts;
    // simtime_picosec _ts;
    static PacketDB<LcpNack> _packetdb;
};
