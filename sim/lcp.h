...
class LcpSrc : public PacketSink, public EventSource, public TriggerTarget {
  ...
  public:
    LcpSrc(LcpLogger *logger, TrafficLogger *pktLogger, EventList &eventList,
    ...
    virtual void connect(Route *routeout, Route *routeback, LcpSink &sink,
...
class LcpSink : public PacketSink, public DataReceiver {
  ...
  private:
    LcpAck::seq_t _cumulative_ack;
    uint64_t _packets;
    uint32_t _srcaddr;
    uint32_t _drops;
    int ack_count_idx = 0;
    string _nodename;
    list<LcpAck::seq_t> _received; // list of packets received OOO
    uint16_t _crt_path;
    const Route *_route;
    ...
    void send_ack(simtime_picosec ts, bool marked, LcpAck::seq_t seqno,
                  LcpAck::seq_t ackno, const Route *rt, const Route *inRoute,
                  int path_id);
    void send_nack(simtime_picosec ts, bool marked, LcpAck::seq_t seqno,
                   LcpAck::seq_t ackno, const Route *rt, int, bool);
    bool already_received(LcpPacket &pkt);
};
