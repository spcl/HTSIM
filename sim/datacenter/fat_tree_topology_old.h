...
void FatTreeTopology::set_tier_parameters(int tier, int radix_up, int radix_down, mem_b queue_up, mem_b queue_down,
                                          int bundlesize, linkspeed_bps linkspeed, int oversub) {
    ...
    _downlink_speeds[tier] =
            linkspeed; // this is the link going downwards from this tier.  up/down linkspeeds are symmetric.
    _oversub[tier] = oversub;
    // xxx what to do about queue sizes
}

// load a config file and use it to create a FatTreeTopology
FatTreeTopology *FatTreeTopology::load(const char *filename, QueueLoggerFactory *logger_factory, EventList &eventlist,
...
FatTreeTopology *FatTreeTopology::load(istream &file, QueueLoggerFactory *logger_factory, EventList &eventlist,
                                       mem_b queuesize, queue_type q_type, queue_type sender_q_type) {
    ...
    FatTreeTopology *ft =
            new FatTreeTopology(no_of_nodes, 0, 0, logger_factory, &eventlist, NULL, q_type, 0, 0, sender_q_type);
...
FatTreeTopology::FatTreeTopology(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
    ...
    if (_link_latencies[TOR_TIER] == 0) {
        ...
        cout << "Fat Tree topology with " << timeAsUs(_link_latencies[TOR_TIER]) << "us Src-ToR links, "
...
void FatTreeTopology::set_custom_params(uint32_t no_of_nodes) {
    ...
    if (_agg_switches_per_pod * _bundlesize[AGG_TIER] != _radix_up[TOR_TIER]) {
        cerr << "Mismatch between TOR up radix " << _radix_up[TOR_TIER] << " and " << _agg_switches_per_pod
             ...
             << " pods " << " with an aggregation switch down radix of " << _radix_down[AGG_TIER] << endl;
...
void FatTreeTopology::init_network() {
    ...
    for (uint32_t j = 0; j < NCORE; j++) {
        ...
        switches_c[j] =
                new FatTreeSwitch(*_eventlist, "Switch_Core_" + ntoa(j), FatTreeSwitch::CORE, j, switch_latency, this);
    }
            ...
            for (uint32_t l = 0; l < _radix_up[AGG_TIER] / _bundlesize[CORE_TIER]; l++) {
                ...
                for (uint32_t b = 0; b < _bundlesize[CORE_TIER]; b++) {
                    ...
                    if ((l + agg * _agg_switches_per_pod) < failed_links) {
                        queues_nc_nup[core][agg][b] = alloc_queue(queueLogger, _downlink_speeds[CORE_TIER] / 10,
                                                                  _queue_down[CORE_TIER], DOWNLINK, CORE_TIER, false);
                        cout << "Adding link failure for agg_sw " << ntoa(agg) << " l " << ntoa(l) << " b " << ntoa(b)
                             << endl;
                    } else {
            ...
            for (uint32_t b_up = 0; b_up < _bundlesize[AGG_TIER]; b_up++) {
                for (uint32_t b_down = 0; b_down < _bundlesize[AGG_TIER]; b_down++) {
                    ...
                    if (reverse) {
                        ...
                        if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                            routeback->push_back(
                                    queues_nlp_nup[HOST_POD_SWITCH(dest)][upper][b_down]->getRemoteEndpoint());
                        ...
                        if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                            routeback->push_back(
                                    queues_nup_nlp[upper][HOST_POD_SWITCH(src)][b_up]->getRemoteEndpoint());
            ...
            for (uint32_t l = 0; l < _radix_up[AGG_TIER] / _bundlesize[CORE_TIER]; l++) {
                ...
                for (uint32_t b1_up = 0; b1_up < _bundlesize[AGG_TIER]; b1_up++) {
                    for (uint32_t b1_down = 0; b1_down < _bundlesize[AGG_TIER]; b1_down++) {
                        // b1_up is link number in upgoing bundle from tor to agg, b1_down is link number in downgoing
                        // bundle

                        for (uint32_t b2_up = 0; b2_up < _bundlesize[CORE_TIER]; b2_up++) {
                            for (uint32_t b2_down = 0; b2_down < _bundlesize[CORE_TIER]; b2_down++) {
                                // b2_up is link number in upgoing bundle from agg to core, b2_down is link number in
                                // downgoing bundle note: no bundling supported between host and tor - just use link
                                // number 0
                                // upper is nup
                                ...
                                if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                                    routeout->push_back(
                                            queues_nlp_nup[HOST_POD_SWITCH(src)][upper][b1_up]->getRemoteEndpoint());
