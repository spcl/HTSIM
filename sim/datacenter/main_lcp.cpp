...
int main(int argc, char **argv) {
    ...
    int no_of_conns = 0, cwnd = MAX_CWD_MODERN_LCP, no_of_nodes = DEFAULT_NODES;
    ...
    while (i < argc) {
        if (!strcmp(argv[i], "-o")) {
            ...
            // Saving this for LCP reference, Gbps
            i++;
            ...
            PKT_SIZE_MODERN =
                    packet_size; // Saving this for LCP reference, Bytes
            i++;
            ...
            LINK_DELAY_MODERN = hop_latency /
                                1000; // Saving this for LCP reference, ps to ns
            i++;
            ...
            if (!strcmp(argv[i + 1], "delayA")) {
                ...
                printf("Name Running: LCP Version D\n");
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        lcpSnk->setName("lcp_sink_" + ntoa(src) + "_" + ntoa(dest));
        logfile.writeName(*lcpSnk);
        if (crt->recv_done_trigger) {
            Trigger *trig =
                    conns->getTrigger(crt->recv_done_trigger, eventlist);
            lcpSnk->set_end_trigger(*trig);
        }
        ...
        case REACTIVE_ECN: {
            ...
            lcpSrc->connect(srctotor, dsttotor, *lcpSnk, crt->start);
            lcpSrc->set_paths(number_entropies);
            lcpSnk->set_paths(number_entropies);
        ...
        case SINGLE_PATH: {
            ...
            routeout->add_endpoints(lcpSrc, lcpSnk);

            routein = new Route(
                    *top->get_bidir_paths(dest, src, false)->at(choice));
            routein->add_endpoints(lcpSnk, lcpSrc);
            lcpSrc->connect(routeout, routein, *lcpSnk, crt->start);
            break;
        }
        case NOT_SET: {
            abort();
    ...
    for (size_t ix = 0; ix < lcp_srcs.size(); ix++) {
        new_pkts += lcp_srcs[ix]->_new_packets_sent;
        rtx_pkts += lcp_srcs[ix]->_rtx_packets_sent;
        bounce_pkts += lcp_srcs[ix]->_bounces_received;
    }
    cout << "New: " << new_pkts << " Rtx: " << rtx_pkts
         << " Bounced: " << bounce_pkts << endl;

    for (std::size_t i = 0; i < lcp_srcs.size(); ++i) {
        delete lcp_srcs[i];
    }
}
