...
void FatTreeSwitch::receivePacket(Packet &pkt) {
    ...
    if (_packets.find(&pkt) == _packets.end()) {
        ...
        // cout << "Switch type " << _type <<  " id " << _id << " pkt dst " << pkt.dst() << " dir " <<
        // pkt.get_direction() << endl;
        pkt.sendOn();
    }
};
...
uint32_t FatTreeSwitch::adaptive_route(vector<FibEntry *> *ecmp_set, int8_t (*cmp)(FibEntry *, FibEntry *)) {
    ...
    // cout << "ECMP set choices " << ecmp_set->size() << " Choice count " << best_choices_count << " chosen entry " <<
    // choiceindex << " chosen path " << choice << " ";

    if (cmp == compare_flow_count) {
        // for (uint32_t i = 0; i<best_choices_count;i++)
        //   cout << "pathcnt " << best_choices[i] << "="<< _port_flow_counts[(BaseQueue*)(
        //   (*ecmp_set)[best_choices[i]]->getEgressPort()->at(0))]<< " ";
            ...
            case ADAPTIVE_ROUTING:
                if (_ar_sticky == FatTreeSwitch::PER_PACKET) {
                    ...
                    if (_flowlet_maps.find(pkt.flow_id()) != _flowlet_maps.end()) {
                        ...
                        // and
                        // 50% chance happens.
                        // and (commented out) if the switch has not taken any other placement decision that we've not
                        // seen the effects of.
                        if (eventlist().now() - f->_last > _sticky_delta &&
                            /*eventlist().now() - _last_choice > _pipe->delay() + BaseQueue::_update_period  &&*/
                            random() % 2 == 0) {
                            ...
                            if (fn(available_hops->at(f->_egress), available_hops->at(new_route)) < 0) {
                                f->_egress = new_route;
                                _last_choice = eventlist().now();
                                // cout << "Switch " << _type << ":" << _id << " choosing new path "<<  f->_egress << "
                                // for " << pkt.flow_id() << " at " << timeAsUs(eventlist().now()) << " last is " <<
                ...
                for (uint32_t l = 0; l < uplink_bundles; l++) {
                    ...
                    for (uint32_t b = 0; b < _ft->bundlesize(CORE_TIER); b++) {
                        ...
                        */
                        ...
                        // cout << "AGG switch " << _id << " adding route to " << pkt.dst() << " via CORE " << k << "
                        // bundle_id " << b << endl;
                    }
                }
                //_uproutes = _fib->getRoutes(pkt.dst());
