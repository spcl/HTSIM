...
class FatTreeTopology : public Topology {
  ...
  private:
    ...
    // switch radix used.  Eg _radix_down[0] = 32 indicates 32 downlinks from ToRs.  _radix_up[2] should be zero in a
    // 3-tier topology.
    static uint32_t _radix_down[3];
    static uint32_t _radix_up[2];

    // switch queue size used.  Eg _queue_down[0] = 32 indicates 32 downlinks from ToRs.  _queue_up[2] should be zero in
    // a 3-tier topology.
