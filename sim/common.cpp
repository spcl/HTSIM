...
uint64_t SINGLE_PKT_TRASMISSION_TIME_MODERN = PKT_SIZE_MODERN * 8 / LINK_SPEED_MODERN;
int LINK_DELAY_MODERN = 400; // ns
uint64_t BASE_RTT_MODERN = (HOPS * LINK_DELAY_MODERN) + (PKT_SIZE_MODERN * 8 / LINK_SPEED_MODERN * HOPS) +
                           (HOPS * LINK_DELAY_MODERN) + (40 * 8 / LINK_SPEED_MODERN * HOPS); // Target RTT in ns
// UEC Specific Values
...
uint64_t MIN_K_ECN_MODERN = BDP_MODERN_UEC * 8 / LINK_SPEED_MODERN * 0.2; // 20% BDP
uint64_t MAX_K_ECN_MODERN = BDP_MODERN_UEC * 8 / LINK_SPEED_MODERN * 0.8; // 100% BDP
// NDP values
uint64_t BDP_MODERN_NDP = BASE_RTT_MODERN * LINK_SPEED_MODERN / 8; // BDP
uint64_t MAX_CWD_MODERN_NDP = BDP_MODERN_NDP * 3; // BDP * 3. Based on their choice to use 23 cwd with 9000 pacekts
...
uint64_t BASE_RTT_OLD = (HOPS * LINK_DELAY_OLD) + (PKT_SIZE_OLD * 8 / LINK_SPEED_OLD * HOPS) + (HOPS * LINK_DELAY_OLD) +
