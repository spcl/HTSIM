#ifndef LCPPACKETDROP_H
#define LCPPACKETDROP_H

#include "network.h"
#include <list>

// LcpDropPacket and LcpDropAck are subclasses of Packet.
// They incorporate a packet database, to reuse packet objects that are no
// longer needed. Note: you never construct a new LcpDropPacket or LcpDropAck
// directly; rather you use the static method newpkt() which knows to reuse old
// packets from the database.

class LcpDropPacket : public Packet {
  public:
    typedef uint64_t seq_t;
    packet_direction _trim_direction;

    LcpDropPacket() : Packet(){};

    inline static LcpDropPacket *newpkt(PacketFlow &flow, const Route &route,
                                        seq_t seqno, seq_t dataseqno, int size,
                                        bool retransmitted = false,
                                        uint32_t destination = 99) {
        LcpDropPacket *p = _packetdb.allocPacket();
        p->set_route(
                flow, route, size + acksize,
                seqno + size -
                        1); // The LCP sequence number is the first byte of the
    ...
    inline static LcpDropPacket *newpkt(LcpDropPacket &source) {
        LcpDropPacket *p = _packetdb.allocPacket();
    ...
    void free() {
        // printf("Packet (LcpDropPacket) being freed ID is %d - From %d\n",
        // id(),
        //        from);
        // fflush(stdout);
        _packetdb.freePacket(this);
    }
    virtual ~LcpDropPacket() {}
  ...
  protected:
    ...
    bool _syn;
    simtime_picosec _ts;
    static PacketDB<LcpDropPacket> _packetdb;
    bool _retransmitted;
};

class LcpDropAck : public Packet {
  public:
    typedef LcpDropPacket::seq_t seq_t;

    LcpDropAck() : Packet(){};

    inline static LcpDropAck *newpkt(PacketFlow &flow, const Route &route,
                                     seq_t seqno, seq_t ackno, seq_t dackno,
                                     uint32_t destination = UINT32_MAX) {
        LcpDropAck *p = _packetdb.allocPacket();
    ...
    void free() {
        // printf("Packet (LcpDropAck) being freed ID is %d - From %d\n", id(),
        // from); fflush(stdout);
        _packetdb.freePacket(this);
    }
    inline seq_t seqno() const { return _seqno; }
  ...
  protected:
    ...
    simtime_picosec _ts;
    static PacketDB<LcpDropAck> _packetdb;
};

class LcpDropNack : public Packet {
  public:
    typedef LcpDropPacket::seq_t seq_t;

    LcpDropNack() : Packet(){};

    inline static LcpDropNack *newpkt(PacketFlow &flow, const Route &route,
                                      seq_t seqno, seq_t ackno, seq_t dackno,
                                      uint32_t destination = UINT32_MAX) {
        LcpDropNack *p = _packetdb.allocPacket();
        p->set_route(flow, route, acksize, ackno);
        p->_bounced = false;
        p->_type = LCPNACK_DROP;
        p->_seqno = seqno;
        p->_ackno = ackno;
        p->hop_count = 0;
        p->_data_ackno = dackno;
    ...
    void free() {
        // printf("Packet (LcpDropNack) being freed ID is %d - From %d\n", id(),
        // from); fflush(stdout);
        _packetdb.freePacket(this);
    }
    inline seq_t seqno() const { return _seqno; }
  ...
  protected:
    ...
    simtime_picosec _ts;
    // simtime_picosec _ts;
    static PacketDB<LcpDropNack> _packetdb;
};
