...
// longer needed. Note: you never construct a new NdpPacket or NdpAck directly;
// rather you use the static method newpkt() which knows to reuse old packets
// from the database.

const static int ndp_acksize = 64;
...
class NdpPacket : public Packet {
  ...
  protected:
    seq_t _seqno;
    seq_t _pacerno; // the pacer sequence number from the pull, seq space is
                    // common to all flows on that pacer
    simtime_picosec _ts;
...
class NdpPull : public Packet {
  public:
    ...
    inline static NdpPull *newpkt(NdpAck *ack) {
        ...
        assert(ack->route());
        p->set_route(ack->flow(), *(ack->route()), ndp_acksize, ack->ackno());
    ...
    inline static NdpPull *newpkt(NdpNack *nack) {
        NdpPull *p = _packetdb.allocPacket();
        assert(nack->route());
        p->set_route(nack->flow(), *(nack->route()), ndp_acksize,
                     nack->ackno());
    ...
    inline static NdpPull *newpkt(NdpRTS *rts, seq_t cumack, seq_t pullno) {
        NdpPull *p = _packetdb.allocPacket();
        p->set_attrs(rts->flow(), ndp_acksize, 0);
        ...
        // cout << "Creating PULL packet with pull no " << p->_pullno << "
        // received pull no " << pullno << endl;
        p->_path_len = 0;
        p->_direction = NONE;
        p->set_dst(rts->dst());
        return p;
    ...
    inline static NdpPull *newpkt(NdpRTS *rts, const route_t &route,
        ...
        // cout << "Creating PULL2 packet with pull no " << p->_pullno << "
        // received pull no " << pullno << endl;
