...
LcpPacer::LcpPacer(EventList &event_list, LcpSrc &flow)
        : EventSource(event_list, "generic_pacer"), flow(&flow),
          _interpacket_delay(0) {
    _last_send = eventlist().now();
}

void LcpPacer::schedule_send(simtime_picosec delay) {
...
void LcpPacer::cancel() {
    _interpacket_delay = 0;
    _next_send = 0;
    eventlist().cancelPendingSource(*this);
}

void LcpPacer::just_sent() { _last_send = eventlist().now(); }

void LcpPacer::doNextEvent() {
