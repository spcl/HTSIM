...
LcpSrc::LcpSrc(LcpLogger *logger, TrafficLogger *pktLogger,
    ...
    if (use_pacing && generic_pacer == NULL) {
        generic_pacer = new LcpPacer(eventlist(), *this);
...
void LcpSrc::set_end_trigger(Trigger &end_trigger) {
...
void LcpSrc::updateParams() {
    ...
    printf("UPDATING VALUES - Link Delay %d (InterDC %lu) - Link Speed %lu - "
           ...
           PKT_SIZE_MODERN, _base_rtt, _target_rtt, time_to_drain_queue, _bdp,
           _cwnd, _switch_queue_size, _hop_count, stop_pacing_after_rtt);
    fflush(stdout);
    _max_good_entropies = 10; // TODO: experimental value
    _enableDistanceBasedRtx = false;
    last_pac_change = 0;
...
void LcpSrc::mark_received(LcpAck &pkt) {
    // cummulative ack
    if (pkt.seqno() == 1) {
...
void LcpSrc::set_traffic_logger(TrafficLogger *pktlogger) {
...
void LcpSrc::processNack(LcpNack &pkt) {
...
void LcpSrc::simulateTrimEvent(LcpAck &pkt) {
...
void LcpSrc::processBts(LcpPacket *pkt) {
    _list_cwd.push_back(std::make_pair(eventlist().now() / 1000, _cwnd));
...
void LcpSrc::processAck(LcpAck &pkt, bool force_marked) {
    LcpAck::seq_t seqno = pkt.ackno();
    ...
    if (newRtt > _base_rtt * quickadapt_lossless_rtt && marked &&
        ...
        simulateTrimEvent(dynamic_cast<LcpAck &>(pkt));
    }
...
void LcpSrc::receivePacket(Packet &pkt) {
    ...
    if (_logger) {
        _logger->logLcp(*this, LcpLogger::LCP_RCV);
    }
    switch (pkt.type()) {
    case LCP:
        // BTS
    ...
    case LCPACK:
        ...
        processAck(dynamic_cast<LcpAck &>(pkt), false);
    ...
    case LCPNACK:
        ...
        if (_trimming_enabled) {
            _next_pathid = -1;
            count_received++;
            processNack(dynamic_cast<LcpNack &>(pkt));
            pkt.free();
        }
        break;
    default:
...
void LcpSrc::adjust_window(simtime_picosec ts, bool ecn, simtime_picosec rtt) {
...
void LcpSrc::connect(Route *routeout, Route *routeback, LcpSink &sink,
...
void LcpSrc::send_packets() {
    ...
    while (get_unacked() + _mss <= c && _highest_sent < _flow_size) {
        ...
        uint64_t data_seq = 0;
        LcpPacket *p = LcpPacket::newpkt(_flow, *_route, _highest_sent + 1,
        ...
        _sent_packets.push_back(
                LcpSentPacket(eventlist().now() + service_time + _rto, p->seqno(),
                           false, false, false));
...
void LcpSrc::set_paths(uint32_t no_of_paths) {
    if (_route_strategy != ECMP_FIB && _route_strategy != ECMP_FIB_ECN &&
        ...
        _route_strategy != ECMP_RANDOM_ECN &&
        _route_strategy != ECMP_RANDOM2_ECN) {
        cout << "Set paths lcp (path_count) called with wrong route "
                "strategy "
...
bool LcpSrc::resend_packet(std::size_t idx) {
    ...
    _unacked += _mss;
    LcpPacket *p = LcpPacket::newpkt(_flow, *_route, _sent_packets[idx].seqno,
...
LcpSink::LcpSink() : DataReceiver("sink"), _cumulative_ack{0}, _drops{0} {
    _nodename = "lcpsink";
}

void LcpSink::set_end_trigger(Trigger &end_trigger) {
    _end_trigger = &end_trigger;
}

void LcpSink::send_nack(simtime_picosec ts, bool marked, LcpAck::seq_t seqno,
                        LcpAck::seq_t ackno, const Route *rt, int path_id,
                        bool is_failed) {

    LcpNack *nack =
            LcpNack::newpkt(_src->_flow, *_route, seqno, ackno, 0, _srcaddr);
...
bool LcpSink::already_received(LcpPacket &pkt) {
    LcpPacket::seq_t seqno = pkt.seqno();
...
void LcpSink::receivePacket(Packet &pkt) {
    ...
    LcpPacket *p = dynamic_cast<LcpPacket *>(&pkt);
    LcpPacket::seq_t seqno = p->seqno();
    LcpPacket::seq_t ackno = p->seqno() + p->data_packet_size() - 1;
...
void LcpSink::send_ack(simtime_picosec ts, bool marked, LcpAck::seq_t seqno,
                       LcpAck::seq_t ackno, const Route *rt,
    ...
    case ECMP_RANDOM2_ECN:
    case ECMP_RANDOM_ECN:
        ack = LcpAck::newpkt(_src->_flow, *_route, seqno, ackno, 0, _srcaddr);
...
LcpRtxTimerScanner::LcpRtxTimerScanner(simtime_picosec scanPeriod,
                                       EventList &eventlist)
