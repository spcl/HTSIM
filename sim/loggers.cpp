...
void LoggedManager::add_logged(Logged *logged) { _idmap.push_back(logged); }
...
void MultiQueueLoggerSampling::logQueue(BaseQueue &queue, QueueEvent ev, Packet &pkt) {
    ...
    case PKT_ENQUEUE:
        ...
        // cout << get_id() << " EN size " << pkt.size() << " Queue " <<
        // queue.nodename() << " total " << _currentQueueSizeBytes << " qs " <<
        // queue.queuesize() << " id " << queue.get_id() << endl;
    ...
    case PKT_SERVICE:
        ...
        // cout << get_id() << " SE size " << pkt.size() << " Queue " <<
        // queue.nodename() << " total " << _currentQueueSizeBytes << " qs " <<
        // queue.queuesize() << " id " << queue.get_id() << endl;
...
void SwiftSinkLoggerSampling::doNextEvent() {
    ...
    // if it's the first time we're here, reserve space for subflow sequence
    // numbers
...
void NdpSinkLoggerSampling::doNextEvent() {
    ...
    // uint32_t deltaSnd = 0;
    double rate;
    // cout << "NdpSinkLoggerSampling(p=" << timeAsSec(_period) << "), t=" <<
    // timeAsSec(now) << "\n";
    for (uint64_t i = 0; i < _sinks.size(); i++) {
...
void ReorderBufferLoggerSampling::logBuffer(BufferEvent ev) {
    switch (ev) {
    case BUF_ENQUEUE:
        _queue_len++;
        if (_queue_len > _max_queue)
            _max_queue = _queue_len;
        break;
    case BUF_DEQUEUE:
        _queue_len--;
        if (_queue_len < _min_queue)
            _min_queue = _queue_len;
        assert(_queue_len >= 0);
        break;
    };
...
void UecTrafficLogger::logTraffic(Packet &pkt, Logged &location, TrafficEvent ev) {
    ...
    if (p.type() == UECACK) {
        val3 |= UEC_IS_ACK;
    } else if (p.type() == UECNACK) {
        val3 |= UEC_IS_NACK;
    } else if (p.type() == UEC) {
        UecPacket &np = static_cast<UecPacket &>(pkt);
        if (np.header_only()) {
            val3 |= UEC_IS_HEADER;
        }
    }
...
string UecTrafficLogger::event_to_str(RawLogEvent &event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    assert(event._type == Logger::UEC_TRAFFIC);
